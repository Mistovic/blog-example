...
function main() {
    ...
    publishedPosts[0].comments = [
        new Comment(publishedPosts[0].ID, "Drago mi je da je tako! Sreca je svuda oko nas.", usersAll[0]),
        new Comment(publishedPosts[0].ID, "Ne pricaj gluspoti, ako vidis srecu svuda okolo zaista si slijep!", usersAll[1])
    ];
    ...
    window.addEventListener('click', function (e) {
        ...
        if (el && el.id == 'submit-register') {
            ...
            console.log('New user: ', new User(newName, newSurname, newUsername, newPassword));
